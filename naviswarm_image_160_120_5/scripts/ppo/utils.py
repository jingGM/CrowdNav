    ...
    def __call__(self, x):
        ...
        if len(x) > 1:
            ...
            if self.obstype == "image":
                bridge = CvBridge()
                try:
                  image_now = bridge.imgmsg_to_cv2(x[0].image_now.data, "bgr8")
                except CvBridgeError as e:
                  print(e)
                try:
                  image_p1rev = bridge.imgmsg_to_cv2(x[0].image_p1rev.data, "bgr8")
                except CvBridgeError as e:
                  print(e)
                try:
                  image_p2rev = bridge.imgmsg_to_cv2(x[0].image_p2rev.data, "bgr8")
                except CvBridgeError as e:
                  print(e)
                try:
                  image_p3rev = bridge.imgmsg_to_cv2(x[0].image_p3rev.data, "bgr8")
                except CvBridgeError as e:
                  print(e)
                try:
                  image_p4rev = bridge.imgmsg_to_cv2(x[0].image_p4rev.data, "bgr8")
                except CvBridgeError as e:
                  print(e)
                ...
                if self.delta:
                    ...
                    data = np.array([np.stack((np.array(image_now),np.array(image_p1rev),np.array(image_p2rev),
                                     np.array(image_p3rev),np.array(image_p4rev)),axis=0)])
            else:
                data = np.array(x)
            if self.update:
