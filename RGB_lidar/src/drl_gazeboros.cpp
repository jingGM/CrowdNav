...
//ROS_INFO("------actions------");
      ...
      else
      {
        ...
        std::cout<<"Robot:"<<i<<" Rew: "<<current_transition.reward<<"  AG: "<<reward_approaching_goal<<" CurrSt: "<<penalty_for_time<<" Osc:"<<penalty_for_bigvz<<"  DtO: "<<distance_to_obstacle<<" RW:"<<reached_way_point<<"    DtoC: "<<depthmin<<"  GDist: "<<state.goalObs.goal_now.goal_dist<<" CollStat: "<<std::to_string(collision_status[i])<<std::endl;
