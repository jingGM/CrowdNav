...
class Agent(object):
    ...
    def obs_filter(self, obs):
        ...
        image_filtered = np.array(image_filtered,dtype=float)/255
        print(image_filtered.shape)

        # fig = plt.figure(figsize=(10, 5))
        # ax = fig.add_subplot(131)
        # toplot = image_filtered[0,::,::,0]
        # plt.imshow(toplot)
        # ax = fig.add_subplot(132)
        # toplot = image_filtered[0,::,::,1]
        # plt.imshow(toplot)
        # ax = fig.add_subplot(133)
        # toplot = image_filtered[0,::,::,2]
        # plt.imshow(toplot)
        # plt.savefig('./predictions/{0}.png'.format(self.counter))
        # self.counter +=1
...
class Policy(object):
    ...
    def _policy_net(self):
        ...
        imagenet = tl.layers.DenseLayer(imagenet, n_units=960, act=tf.nn.relu, name='image_dense')
        imagenet = tl.layers.DenseLayer(imagenet, n_units=256, act=tf.nn.relu, name='image_output')
...
class Value(object):
    ...
    def _value_net(self):
        ...
        imagevnet = tl.layers.Conv2dLayer(imagevnet,act=tf.nn.relu,shape=(5, 5, self.obs_shape[3], 64),strides=(1,2,2,1),use_cudnn_on_gpu=True,name='Icnnv1')
        imagevnet = tl.layers.Conv2dLayer(imagevnet,act=tf.nn.relu,shape=(5, 5, 64, 64), strides=(1,1,1,1),use_cudnn_on_gpu=True,name='Icnnv2')
        imagevnet = tl.layers.Conv2dLayer(imagevnet,act=tf.nn.relu,shape=(3, 3, 64, 32), strides=(1,2,2,1),use_cudnn_on_gpu=True,name='Icnnv3')
        imagevnet = tl.layers.FlattenLayer(imagevnet, name='imageflv')
        imagevnet = tl.layers.DenseLayer(imagevnet, n_units=960, act=tf.nn.relu, name='image_densev')
        imagevnet = tl.layers.DenseLayer(imagevnet, n_units=256, act=tf.nn.relu, name='image_voutput')
