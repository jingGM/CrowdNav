...
class Policy(object):
    ...
    def _policy_net(self):
        #scan = tf.placeholder(tf.float32, [None, 3, 80],'scan_ph')
        ...
        image= tf.placeholder(tf.float32, [None, 3, 150], 'image_ph')
        ...
        # image_output = net.outputs

        net = tl.layers.InputLayer(image, name='image_input')
        net = tl.layers.Conv1dLayer(net, act=tf.nn.relu, shape=[5, self.obs_shape[4], 32], stride=2,name='cnn1')
        net = tl.layers.Conv1dLayer(net, act=tf.nn.relu, shape=[3, 32, 16], stride=2,name='cnn2')
        net = tl.layers.FlattenLayer(net, name='fl')
        net = tl.layers.DenseLayer(net, n_units=256, act=tf.nn.relu, name='image_output')
        image_output = net.outputs
        ...
        return [act_net,net, linear, angular], [ goal, vel, image],action_mean, log_vars, sampled_act, test_act
...
class Value(object):
    ...
    def _value_net(self):
        ...
        netv = tl.layers.InputLayer(image, name='image_input')
        netv = tl.layers.Conv1dLayer(netv, act=tf.nn.relu, shape=[5, self.obs_shape[4], 32], stride=2,name='cnn1v')
        netv = tl.layers.Conv1dLayer(netv, act=tf.nn.relu, shape=[3, 32, 16], stride=2,name='cnn2v')
        netv = tl.layers.FlattenLayer(netv, name='flv')
        netv = tl.layers.DenseLayer(netv, n_units=256, act=tf.nn.relu, name='image_output_value')
        image_voutput = netv.outputs
        ...
        return [value_net,netv], [goal, vel, image], value
