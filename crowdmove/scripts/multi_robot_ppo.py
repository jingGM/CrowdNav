...
class MultiRobotDRL(object):
    ...
    def _rollout(self, env):
        ...
        for step in range(args.episode_max_steps):
            ...
            if args.train:
                action_agents = self.agent.policy.act([scan_input, goal_input, vel_input], terminateds)
            else:
                action_agents = self.agent.policy.act_test([scan_input, goal_input, vel_input], terminateds)
