...
class Policy(object):
    ...
    def _policy_net(self):
        ...
        net = tl.layers.Conv1dLayer(
            net, act=tf.nn.relu, shape=[5, self.obs_shape[0], 32], stride=2,
            name='cnn1')
        net = tl.layers.Conv1dLayer(
            net, act=tf.nn.relu, shape=[3, 32, 16], stride=2,
            name='cnn2')
        net = tl.layers.FlattenLayer(net, name='fl')
        net = tl.layers.DenseLayer(
            net, n_units=256, act=tf.nn.relu, name='cnn_output')
        cnn_output = net.outputs

        def my_sigmoid(x):
            return self.max_vx * tf.nn.sigmoid(x)

        act_net = tl.layers.InputLayer(
            tf.concat([goal, vel, cnn_output], axis=1), name='goal_input')
        act_net = tl.layers.DenseLayer(
            act_net, n_units=128, act=tf.nn.relu, name='act1')
        linear = tl.layers.DenseLayer(
            act_net, n_units=1, act=my_sigmoid, name='linear')
...
class Value(object):
    ...
    def _value_net(self):
        ...
        value_net = tl.layers.DenseLayer(value_net, n_units=128, act=tf.nn.relu, name='value1')
...
if __name__ == '__main__':
    session = tf.Session()
    agent = Agent(session, [3, 512, 2, 2, 1], 2)
