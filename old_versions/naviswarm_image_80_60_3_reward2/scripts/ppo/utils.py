    ...
    def __call__(self, x):
        ...
        if len(x) > 1:
            ...
            for i in range(len(x)):  # for each agent
                if self.obstype == "scan":
                    ...
                    if self.delta:
                        ...
                        data = np.stack((np.array(image_now),np.array(image_p1rev),np.array(image_p2rev)),axis=0)
                        #print(data.shape)
                        #print('==========image shape======================')
            ...
            if self.obstype == "image":
                ...
                data = np.array([np.stack((np.array(image_now),np.array(image_p1rev),np.array(image_p2rev)),axis=0)])
            else: 
                data = np.array(x)
