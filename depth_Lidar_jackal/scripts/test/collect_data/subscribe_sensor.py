...
def testcompressed():
    ...
    rospy.Subscriber("/turtlebot0/scan_filtered", LaserScan, callbackfortest,callback_args=count, queue_size=1)
    
    rospy.spin()
   


def callbackfortest(data,count):
    lidarrange = [data.ranges[i] for i in range(len(data.ranges))]
    if not count[0]:
        with open('document.csv','a') as fd:
            writer = csv.writer(fd)
            writer.writerow(lidarrange)  
        print(lidarrange)
        count[0] = True
