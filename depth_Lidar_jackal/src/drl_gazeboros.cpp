...
class GazeboTrain {
  private:
    ...
    std::vector<std::vector<bool>> waypoint_flag;
...
GazeboTrain::GazeboTrain(int n){
  ...
  std::vector<bool> waypoint_flag_;
  ...
  for(int i=0;i<n;i++){
    ...
    waypoint_flag.push_back(waypoint_flag_);
    step_number.push_back(0);
  }
  ...
  if (resettingflag){}
    ...
    for (int j=0; j< num_robots;j++){
      for (int i = 0; i < gt.name.size(); i++){
        std::string current_robot_name ="jackal"+std::to_string(j);
...
bool GazeboTrain::cb_update_srv(naviswarm::UpdateModelRequest& request, naviswarm::UpdateModelResponse& response)
{
    ...
    if (request.points.size() != num_robots) {
        ...
        for (size_t r = 0; r < request.points.size(); r++) {
            ...
            for (auto i : waypoint_data[r].data){
                waypoint_flag[r].push_back(false);
// std::cout<<waypoint_flag[r].size()<<"------"<<waypoint_data[r].data.size()<<std::endl;
            }
        }
...
int GazeboTrain::create_sharedmemory(){
  ...
  for (int i = 0; i < num_robots; ++i)
    {
      std::string name_space = "/jackal" + std::to_string(i);
...
int GazeboTrain::train(){
    ...
    for(int i = 0; i < num_robots; i++){
        ...
        else // if goal has not been reached
        {
            ...
            if(collision_status[i] == true) { // stalled is obtained from an in-built function from stage. we must write a function to detect collisions
              ...
              for (int waypoint_index =0;waypoint_index<waypoint_data[i].data.size();waypoint_index++){
                ...
                if (distance_to_waypoint<0.4 && waypoint_flag[i][waypoint_index]==false){
                    reached_way_point =10;
                    waypoint_flag[i][waypoint_index]= true;
                }
              }
              ...
              penalty_for_time = (step_number[i]) *(0);
        ...
        std::cout<<"Robot:"<<i<<" Rew: "<<current_transition.reward<<"  AG: "<<reward_approaching_goal<<" CurrSt: "<<penalty_for_time<<" Osc:"<<penalty_for_bigvz<<"  DtO: "<<distance_to_obstacle<<" RW:"<<reached_way_point<<"  GDist: "<<state.goalObs.goal_now.goal_dist<<" CollStat: "<<std::to_string(collision_status[i])<<" Steps: "<<step_number[i]<<std::endl;
  ...
  */
    ...
    while (!succ && ros::ok()){
      ...
      if (succ)
      {
          ...
          //usleep(300000);
          for (int j = 0 ; j < num_robots; ++j){
            std::string topicname = "/jackal"+std::to_string(j)+"/jackal_velocity_controller/cmd_vel";
