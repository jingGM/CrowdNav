...
def get_policy_from_params(policy_params, env, *args, **kwargs):
    ...
    observation_preprocessors_params = policy_kwargs.pop('observation_preprocessors_params', {})
    observation_keys = policy_kwargs.pop('observation_keys', None) or env.observation_keys

    observation_shapes = OrderedDict(((key, value) for key, value in env.observation_shape.items()
    ...
    for name, observation_shape in observation_shapes.items():
        ...
        observation_preprocessors[name] = get_preprocessor_from_params(env, preprocessor_params)
