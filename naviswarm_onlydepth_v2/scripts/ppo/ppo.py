...
class PPO(object):
    def __init__(self, args, agent, session, obs_shape, ac_shape):
        ...
        self.replay_buffer_obs_image = None
        self.critic_epochs = args.critic_epochs
        self.critic_lr = args.critic_lr

        self.obs_goal, self.obs_vel, self.obs_image = agent.policy.obs
        self.actor = agent.policy
        self.means = agent.policy.means
        self.log_vars = agent.policy.log_vars

        self.obs_goal_value, self.obs_vel_value, self.obs_image_value =agent.value.obs
    ...
    def update(self, paths):
        ...
        baseline_value = self.baseline.predict([obs_goal, obs_vel,obs_image])
        ...
        if self.time_step > 1: # train acotr after trained critic
            kl = self.actor_update(obs_image, obs_goal, obs_vel, acts, advs)
        self.critic_update(obs_image, obs_goal, obs_vel, rets)
    ...
    def actor_update(self,obs_image, obs_goal, obs_vel, acts, advs):
        feed_dict = {
            #self.obs_scan: obs_scan,
            self.obs_image:obs_image,
            self.obs_goal: obs_goal,
    ...
    def critic_update(self,obs_image, obs_goal, obs_vel, rets):
        ...
        for e in range(self.critic_epochs):
            obs_goal_train, obs_vel_train,obs_image_train, ret_train = shuffle(obs_goal_train, obs_vel_train,obs_image_train, ret_train)
            for j in range(num_batches):
                ...
                self.baseline.update([obs_goal_set, obs_vel_set,obs_image_set], ret_set)
