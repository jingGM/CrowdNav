...
# Other python programs in the package
...
class StageEnv(object):
    ...
    def __init__(self, num_agents=2, num_obstacles=0,
        ...
        # Defining Markers for Viz in Rviz
        ...
        # This will be subscribed by Rviz
    ...
    # This function modifies the action generated by this program then writes it into shared memory
    def step(self, actions):
        ...
        for action in actions:
            # Limiting the lin and ang velocities
        ...
        actions_probuf = Actions() # Action buffer
        for a in actions:
            apb = Action()
            apb.vx = a[0]
            apb.vz = a[1]
    ...
    def reset(self):
        ...
        """
        # Delete the previously used markers and publish that
        ...
        succ = False
        while not succ:
            succ = self._update_stage() # function defined later
        ...
        obs, _, _, _ = self._read_data() # previously defined
    ...
    # Returns the straight line distance between start and goal for all agents.
    ...
    # Adds distances between successive waypoints for all agents
    def get_trajectory(self):
        delta = np.array(self.agent_poses) - np.array(self.agent_last_poses) # Change in distance. (Straight line model)
        distance = np.hypot(
            delta[:self.num_agents, 0], delta[:self.num_agents, 1])
    ...
    # NOTE: Need to edit this function
    def _update_stage(self):
        self.starts = []
        self.goals = []

        self.scenarios.reset() # Initializes start and goals to empty arrays
    ...
    def _add_marker(self,
        ...
        if ns == "usv":
            ...
            marker.mesh_resource = "file:///home/jing/Documents/catkin_workspace/catkin_CamNavi/src/naviswarm/rviz/usv.dae"
            marker.id = self.agent_markers_id
            self.agent_markers.markers.append(marker)
            self.agent_markers_id += 1
