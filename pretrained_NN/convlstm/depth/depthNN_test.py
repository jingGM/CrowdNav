...
for imgindex in range(321,348):
    index = 1*(imgindex-321)+321
    imagenow = np.genfromtxt("./csvs/ground{0:06d}.csv".format(index), delimiter=',')
    imagenow = np.array(cv2.resize(imagenow, (80, 60)), dtype=np.float)
    imagenow = imagenow /maxpixelvalue
    imagenow = np.expand_dims(np.array(imagenow),axis=2)

    print(imagenow.shape)
    imagenext = np.genfromtxt("./csvs/ground{0:06d}.csv".format(index+2), delimiter=',')
    imagenext = np.array(cv2.resize(imagenext, (80, 60)), dtype=np.float)
    imagenext = imagenext /maxpixelvalue
    ...
    imagesinput = imagesinput/imagesinput.max()
    ...
    toplot = imagenext[::,::,0]
    plt.imshow(toplot)
    plt.savefig('predictions_{0}.png'.format(imgindex))

    np.savetxt("./csvspredict/{0}.csv".format(imgindex), imageoutput[0,0,::,::,0], delimiter=",")
    np.savetxt("./csvspredict/ground{0}.csv".format(imgindex), imagenext[::,::,0], delimiter=",")
