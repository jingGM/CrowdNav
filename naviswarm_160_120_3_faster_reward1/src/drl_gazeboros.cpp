      ...
      else
      {
          ...
          else // if goal has not been reached
          {
              ...
              if(collision_status[i] == true) { // stalled is obtained from an in-built function from stage. we must write a function to detect collisions
                  ...
                  double reward_approaching_goal = 100*(state.goalObs.goal_prev.goal_dist - state.goalObs.goal_now.goal_dist);
                  ...
                  // current_transition.reward = 0;
                  current_transition.reward = reward_approaching_goal + (current_steps+1) *(-0.02) + std::abs(state.velObs.vel_now.vz) * (-0.02);
        ...
        std::cout<<"robot:"<<current_robot<<"  "<<reward.reward_approaching_goal<<" /"<<(current_steps+1) *(-0.02)<<" /"<<std::abs(state.velObs.vel_now.vz) * (-0.02)<<"  /"state.goalObs.goal_now.goal_dist<<std::endl;
        ROS_INFO("-----------");
