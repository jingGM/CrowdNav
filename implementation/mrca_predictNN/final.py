...
class MLSHAgent(object):
    ...
    def _build_ph(self):
        self.image_ph= tf.placeholder(tf.float32,[None, 120,150, 2],'image_ph')
...
class PPOAgent(object):
    ...
    def _build_net(self):
        ...
        imagenet = tl.layers.Conv2dLayer(imagenet,act=tf.nn.relu,shape=(5, 5, 2, 64), strides=(1,2,2,1),use_cudnn_on_gpu=True,name='Icnn1')
...
class Environment(object):
    def __init__(self, agent):
        ...
        self.imagenetwork = Memory(self.detargs, self.trackargs)
    ...
    def process_depth_image(self,imagedata):
        ...
        predictImage = self.imagenetwork.predict(image_now)
        output = cv2.resize(output,dsize=(150,120))
        image_out = np.stack((output,image_now),axis=2)
